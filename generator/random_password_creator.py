...
class Password:
    ...
    def __init__(self, charset, length):
        #  charset is string,length is string length
    ...
    def set_the_charset(self):
        # for character set for password generator
    ...
    def generate_password(self):
        ...
        for i in range(self.length):
            #  for list element ["asdf","1233"]
            outer_index = random.randrange(0, len(self.char_array))
            # select character from element asdf randomly select any character and store
...
#  ''.join is used for remove list square bracket,without it it is print in list format
